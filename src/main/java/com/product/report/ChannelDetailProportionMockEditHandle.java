...
public class ChannelDetailProportionMockEditHandle extends AbstractRestResultHandle {
	...
	public ResultBean responsePageByJson(HttpServletRequest req,
		...
		try 
		{
			...
			if(bean.isModify())
			{
				reportQueryService.modifyChannelDetailProportionMock(bean);
			}
			else if(bean.isDelete() )
			{
				reportQueryService.deleteChannelDetailProportionMock(bean);
			}
			else
			{
				reportQueryService.addChannelDetailProportionMock(bean);
			}
