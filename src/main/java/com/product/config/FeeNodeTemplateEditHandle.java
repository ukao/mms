...
public class FeeNodeTemplateEditHandle extends AbstractRestResultHandle {
	...
	public ResultBean responsePageByJson(HttpServletRequest req,
		...
		try 
		{
			...
			if(bean.isModify())
			{
				configService.modifyFeeNodeTemplate(bean);
			}
			else if(bean.isDelete() )
			{
				configService.deleteFeeNodeTemplate(bean);
			}
			else
			{
				configService.addFeeNodeTemplate(bean);
			}
