...
public class SdkConfigEditHandle extends AbstractRestResultHandle {
	...
	public ResultBean responsePageByJson(HttpServletRequest req,
		...
		try 
		{
			bean = this.createBaseBean(req, SdkConfig.class);
			if(bean.isModify())
			{
				configService.modifySdkConfig(bean);
			}
			else if(bean.isDelete() )
			{
				configService.deleteSdkConfig(bean);
			}
			else
			{
				configService.addSdkConfig(bean);
			}
